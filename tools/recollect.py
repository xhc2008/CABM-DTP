...
def get_notes_db():
    ...
    if _notes_db is None:
        ...
        # 加载现有笔记数据
        notes_file = os.path.join('data', 'memory.json')
...
def recollect(query: str, top_k: int = 5) -> Dict[str, Any]:
    """
    从笔记数据库中检索相关信息
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"回忆时发生错误: {str(e)}",
            "results": []
        }

# 标记为工具函数
recollect.is_tool = True
recollect.tool_definition = tool_definition